...
function findWasmBinary() {
  return locateFile('objbuild.wasm');
}
 ...
 */
  ...
  var _emscripten_glTexEnvi = (target, pname, params) => warnOnce('glTexEnvi: TODO');
  ...
  var _emscripten_glVertex4fv = (p) => _glVertex4f(HEAPF32[((p)>>2)],
                                    ...
                                    HEAPF32[(((p)+(8))>>2)],
                                    HEAPF32[(((p)+(12))>>2)]);
  var _glVertex4fv = _emscripten_glVertex4fv;
...
var ASM_CONSTS = {
  107792: ($0) => { if (Module.streamAsset) Module.streamAsset(UTF8ToString($0)); },  
 107858: ($0) => { if (Module.streamAsset) Module.streamAsset(UTF8ToString($0)); },  
 107924: ($0) => { if (Module.streamAsset) Module.streamAsset(UTF8ToString($0)); },  
 107990: ($0) => { if (Module.streamVolList) { var t = UTF8ToString($0); Module.streamVolList([t + 'World.vol', t + 'Terrain.vol', t + 'DML.vol']); } }
};
function wasmNetOpen(url) { var u = UTF8ToString(url); if (typeof location !== 'undefined' && location.hostname) { var sep = u.indexOf('://'); if (sep >= 0) { var rest = u.substring(sep + 3); var slash = rest.indexOf('/'); var hostport = slash >= 0 ? rest.substring(0, slash) : rest; var tail = slash >= 0 ? rest.substring(slash) : ''; var colon = hostport.indexOf(':'); var port = colon >= 0 ? hostport.substring(colon) : ''; u = u.substring(0, sep + 3) + location.hostname + port + tail; } } var n = (Module.__net = { queue: [], ready: false, ws: null }); var ws = new WebSocket(u); ws.binaryType = "arraybuffer"; ws.onopen = function() { n.ready = true; }; ws.onclose = function() { n.ready = false; }; ws.onerror = function() { n.ready = false; }; ws.onmessage = function(ev) { var b = new Uint8Array(ev.data); if (b.length < 1) return; var L = b[0]; var addr = ""; for (var i = 0; i < L; i++) addr += String.fromCharCode(b[1 + i]); n.queue.push({ addr: addr, data: b.slice(1 + L) }); }; n.ws = ws; }
function wasmNetReady() { return (Module.__net && Module.__net.ready) ? 1 : 0; }
function wasmNetSendTo(ipPort,data,len) { var n = Module.__net; if (!n || !n.ws || n.ws.readyState !== 1) return -1; var addr = UTF8ToString(ipPort); var L = addr.length & 0xff; var msg = new Uint8Array(1 + L + len); msg[0] = L; for (var i = 0; i < L; i++) msg[1 + i] = addr.charCodeAt(i) & 0xff; msg.set(HEAPU8.subarray(data, data + len), 1 + L); n.ws.send(msg); return len; }
function wasmNetRecvFrom(addrOut,addrCap,dataOut,dataCap) { var n = Module.__net; if (!n || n.queue.length === 0) return -1; var pkt = n.queue.shift(); var a = pkt.addr; var alen = Math.min(a.length, addrCap - 1); for (var i = 0; i < alen; i++) HEAPU8[addrOut + i] = a.charCodeAt(i) & 0xff; HEAPU8[addrOut + alen] = 0; var dlen = Math.min(pkt.data.length, dataCap); HEAPU8.set(pkt.data.subarray(0, dlen), dataOut); return dlen; }
...
function assignWasmExports(wasmExports) {
  ...
  _wasmGetDropReason = Module['_wasmGetDropReason'] = wasmExports['wasmGetDropReason'];
  _wasmGetEvtCap = Module['_wasmGetEvtCap'] = wasmExports['wasmGetEvtCap'];
  _wasmGetEvtHist = Module['_wasmGetEvtHist'] = wasmExports['wasmGetEvtHist'];
  _wasmGetEvtTagRaw = Module['_wasmGetEvtTagRaw'] = wasmExports['wasmGetEvtTagRaw'];
  _wasmSetSkip = Module['_wasmSetSkip'] = wasmExports['wasmSetSkip'];
  _wasmSetSkipTag = Module['_wasmSetSkipTag'] = wasmExports['wasmSetSkipTag'];
  _wasmGetSkipHits = Module['_wasmGetSkipHits'] = wasmExports['wasmGetSkipHits'];
  _wasmClearEvtHist = Module['_wasmClearEvtHist'] = wasmExports['wasmClearEvtHist'];
  _wasmGetHealCount = Module['_wasmGetHealCount'] = wasmExports['wasmGetHealCount'];
  _wasmArmEvtTagCap = Module['_wasmArmEvtTagCap'] = wasmExports['wasmArmEvtTagCap'];
  _wasmGetEvtTagCap = Module['_wasmGetEvtTagCap'] = wasmExports['wasmGetEvtTagCap'];
  _wasmArmEvtSection = Module['_wasmArmEvtSection'] = wasmExports['wasmArmEvtSection'];
  _wasmGetEvtSection = Module['_wasmGetEvtSection'] = wasmExports['wasmGetEvtSection'];
  _wasmGetSeqDiag = Module['_wasmGetSeqDiag'] = wasmExports['wasmGetSeqDiag'];
  _wasmDecodeStringAt = Module['_wasmDecodeStringAt'] = wasmExports['wasmDecodeStringAt'];
  _wasmDrainPalette = Module['_wasmDrainPalette'] = wasmExports['wasmDrainPalette'];
  _wasmDrainTerrain = Module['_wasmDrainTerrain'] = wasmExports['wasmDrainTerrain'];
  _wasmGetDnetDiag = Module['_wasmGetDnetDiag'] = wasmExports['wasmGetDnetDiag'];
  _wasmClearDnetDiag = Module['_wasmClearDnetDiag'] = wasmExports['wasmClearDnetDiag'];
